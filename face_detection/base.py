...
# Function to check if the input image meets the required specifications
...
class Detector(ABC):
    ...
    def detect(
        ...
        """
        Takes an RGB image and performs and returns a set of bounding boxes as detections.
    ...
    def _detect(self, image: torch.Tensor) -> torch.Tensor:
        """
        Takes N RGB image and performs and returns a set of bounding boxes as detections.
    ...
    def batched_detect(
        ...
        Returns:
            np.ndarray: a list with N set of bounding boxes of shape [B, 5] with (xmin, ymin, xmax, ymax, score)
        """
        check_image(image)
        height, width = image.shape[1:3]
    ...
    def validate_detections(self, boxes: typing.List[np.ndarray]):
        """
        Validate the detected bounding boxes.
        
        Args:
            boxes (list): List of detected bounding boxes
        
        Raises:
            AssertionError: If confidence values are not within [0, 1]
        """
        for box in boxes:
